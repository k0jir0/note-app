...
Built with Node.js, Express, MongoDB, Mongoose, EJS, Bootstrap, vanilla JavaScript, Passport, bcrypt, express-session, connect-mongo, and Helmet, it combines a unified Research Workspace with encrypted application data and authenticated same-origin media delivery, featuring dedicated Security Operations, Alert Triage ML, Mission Access Assurance, Hardware-Backed MFA, Session Security, Break-Glass Runtime, Query Injection Prevention, XSS and CSP Defense, Server Access Control, Audit Trail and Telemetry, Supply Chain Security, Playwright Testing, Selenium Testing, and Self-Healing Locator Repair modules. The platform also incorporates Google OAuth, WebAuthn, PKI-aware assurance, Redis-backed realtime flows, Docker and distroless container hardening, Nginx sandbox edge isolation, Kubernetes rotation paths, PM2 process management, CycloneDX SBOM generation, Falco and Trivy automation runners, Semgrep and OWASP ZAP security scanning, and Mocha, Chai, Sinon, Playwright, and Selenium WebDriver test coverage, backed by hardened deployment, audit, and repository-control workflows.
