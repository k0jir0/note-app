...
async function fetchWithCookies(path, opts = {}) {
    ...
    try {
        const rawHeaders = response.headers.raw && response.headers.raw()['set-cookie'];

        if (rawHeaders) {
            updateCookiesFromHeaders(rawHeaders);
        }
    } catch (error) {
        void error;
    }
...
(async () => {
    try {
        ...
        if (!loginCsrfMatch) {
            console.error('No CSRF on login page');
            process.exit(1);
        }
        ...
        if (!metaCsrf) {
            console.error('No meta csrf token on /notes');
            process.exit(1);
        }
