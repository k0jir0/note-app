...
(async function main() {
    ...
    // Spies / counters
        ...
        metrics.intrusionIngestCounter.inc = (labels, n) => {
            intrusionCalls.push({ labels, n });
        };
        ...
        assert.strictEqual(res.created, true, 'intrusion should be created');
        assert.strictEqual(res.createdCount, 2, 'should create two alerts');
        assert.strictEqual(intrusionCalls.length, 2, 'metrics.inc should be called for each inserted alert');

        // 2) Test scan ingestion
        const jsonScan = JSON.stringify([
            { id: 'v1', severity: 'high', title: 'Critical vuln' }
        ]);
        ...
        assert.strictEqual(scanRes.created, true, 'scan should be created');
        assert.strictEqual(scanIncCalls.length, 1, 'scanImportCounter.inc should be called once');
        assert.strictEqual(scanGaugeCalls.length, 1, 'scanFindingsGauge.set should be called once');
        assert.strictEqual(scanGaugeCalls[0].value, 1, 'gauge should be set to 1 finding');
